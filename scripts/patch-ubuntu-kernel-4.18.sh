...
# Construct branch name from distribution codename {cosmic,..} and kernel version
...
kernel_branch=$(uname -r | awk -F '[.-]' '{print "v"$1"."$2}') #removed second minor to get the right git branch
kernel_major_minor=$(uname -r | awk -F '[.-]' '{print "v"$1"."$2}')
kernel_name="ubuntu-${ubuntu_codename}-$kernel_branch"


#Distribution-specific packages DONT KWON WHAT SHOULD STAND HERE
if [ ${ubuntu_codename} == "cosmic" ];
then
	require_package libglfw3-dev
	require_package libgl1-mesa-dev
